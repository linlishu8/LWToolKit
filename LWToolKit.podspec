...
长期可用的轻量 iOS 基础工具包，包含节流/防抖、任务队列、Keychain、缓存、本地化、AB 实验、通知、深链路由、UI 组件（Toast/Alert）、媒体选择与加载、事件上报，以及基于 Alamofire 的网络层等模块。
  ...
  s.subspec "LWCore" do |ss|
    ss.source_files = "LWToolKit/Sources/LWCore/**/*.{h,m,swift,mm}"
    ss.frameworks = %w(UIKit UserNotifications Network Security)
    # 如果用到了 AppTrackingTransparency / UniformTypeIdentifiers，也可按需加上：
    # ss.frameworks = %w(UIKit UserNotifications Network Security AppTrackingTransparency UniformTypeIdentifiers)
  end

  s.subspec "LWUI" do |ss|
    ss.dependency "LWToolKit/LWCore"
    ss.source_files = "LWToolKit/Sources/LWUI/**/*.{h,m,swift,mm}"
    ss.frameworks = %w(UIKit)
  end

  s.subspec "LWMedia" do |ss|
    ss.dependency "LWToolKit/LWCore"
    ss.source_files = "LWToolKit/Sources/LWMedia/**/*.{h,m,swift,mm}"
    ss.frameworks = %w(PhotosUI UniformTypeIdentifiers)
  end

  s.subspec "LWAnalytics" do |ss|
    ss.dependency "LWToolKit/LWCore"
    ss.source_files = "LWToolKit/Sources/LWAnalytics/**/*.{h,m,swift,mm}"
  end

  s.subspec "LWNetwork" do |ss|
    ss.dependency "LWToolKit/LWCore"
    ss.dependency "Alamofire", "~> 5.8"
    ss.source_files = "LWToolKit/Sources/LWNetwork/**/*.{h,m,swift,mm}"
