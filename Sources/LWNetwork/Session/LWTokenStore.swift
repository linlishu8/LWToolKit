...
public actor LWTokenStore { public static let shared = LWTokenStore()
  ...
  public func bootstrap(_ token: Token) async { self.token = token; await LWNetKeychain.shared.saveToken(token) }
  ...
  public func update(_ new: Token) async { self.token = new; await LWNetKeychain.shared.saveToken(new) }
